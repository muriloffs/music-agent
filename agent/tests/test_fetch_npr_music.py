...
import pytest
from agent.scripts.fetch_npr_music import fetch


FIXTURE = Path(__file__).parent / "fixtures" / "npr_music_sample.xml"


def test_fetch_npr_music_parses_real_rss():
    xml = FIXTURE.read_text(encoding="utf-8")
    with patch("agent.scripts.fetch_npr_music.http_get_with_retries", return_value=xml):
        items = fetch(data_dir=Path("/tmp/fake"))
    assert len(items) > 0
    item = items[0]
    assert item["fonte_id"] == "npr_music"
...
def test_fetch_npr_music_falls_back_to_cache_when_http_fails(tmp_path):
    cache_report = {
        "fontes_usadas": [],
        "cards": [{
            "id": "c1", "artista": "Cached Artist", "titulo": "Cached Album",
            "tipo": "album", "bucket": "alinhado",
            "fontes_cobertura": [{"id": "npr_music", "url": "https://x.com/1", "tipo": "review"}]
        }]
    }
    import json
    (tmp_path / "relatorio-2026-05-16.json").write_text(json.dumps(cache_report))
    with patch("agent.scripts.fetch_npr_music.http_get_with_retries", return_value=None):
